...
def launch_setup(context, *args, **kwargs):
    ...
    aruco_double_params = {
        ...
        'parent_name': 'stereo_gazebo_camera_optical_frame',
...
def generate_launch_description():
    ...
    marker_id1_arg = DeclareLaunchArgument(
        'marker_id1', default_value='0',
        description='Marker ID. '
    )

    marker_id2_arg = DeclareLaunchArgument(
        'marker_id2', default_value='1',
        description='Marker ID 2. '
    )

    marker_size_arg = DeclareLaunchArgument(
        'marker_size', default_value='0.024', # the tag is 0.03 big but has white space of 0.003 on each side, making the tag 0.024 big 
